...
export function getLatestIssueTimestamp(issues: NormalizedIssue[]) {
  let latest = 0;

  for (const issue of issues) {
    const candidate = Date.parse(issue.updated || issue.created || "");
...
export function buildJiraSyncJql(args: {
  ...
  const archiveThresholdDays = args.archiveThresholdDays || JIRA_ARCHIVE_THRESHOLD_DAYS;
